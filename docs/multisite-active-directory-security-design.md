...
**Source:** Coursework-derived case study, independently rewritten  
**Status:** Case study complete; independent lab revalidation and sanitized screenshots pending  
**Environment:** Fictionalized for safe publication
...
Each site receives a domain controller and a dedicated subnet. The publishable rebuild will use example ranges reserved for documentation:
...
Exact values will be revalidated against current organizational requirements before the independent rebuild. A lab value is not automatically a production recommendation.
...
The independent rebuild will verify:

- DNS resolution and domain authentication from every site
- AD replication health
- OU-scoped delegation using a nonprivileged test account
- GPO application with `gpresult`
- Account-lockout and endpoint-policy behavior using fictional users
- Restoration of a deleted test object
